<x-guest-layout>
    ...
    <form method="POST" action="{{ route('login') }}" class="mt-7 space-y-5">@csrf
        ...
        <div><div class="flex items-center justify-between"><label class="label">Password</label>@if(Route::has('password.request'))<a href="{{ route('password.request') }}" class="mb-2 text-xs font-bold text-indigo-600">Lupa password?</a>@endif</div><input name="password" type="password" class="input" placeholder="Masukkan password" required autocomplete="current-password"></div>
