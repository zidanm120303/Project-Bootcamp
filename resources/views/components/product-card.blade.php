...
<article class="group overflow-hidden rounded-2xl border border-slate-200 bg-white transition duration-300 hover:-translate-y-1 hover:border-indigo-200 hover:shadow-xl hover:shadow-indigo-100/60">
    <a href="{{ route('products.show', $product) }}" class="relative block aspect-[4/3] overflow-hidden bg-slate-100">
        ...
        <span class="absolute left-3 top-3 rounded-lg bg-white/95 px-2.5 py-1 text-[11px] font-extrabold text-indigo-700 shadow-sm">{{ ['rental' => 'Barang Sewa', 'sale' => 'Produk Jual', 'service' => 'Jasa'][$product->product_type] }}</span>
    </a>
    <div class="p-4">
        <div class="flex items-center gap-1.5 text-xs text-slate-500"><x-icon name="store" class="h-4 w-4" /><span class="truncate">{{ $product->partner->business_name }}</span>@if($product->partner->is_trusted)<span title="Mitra Terpercaya" class="text-emerald-500"><x-icon name="shield" class="h-4 w-4" /></span>@endif</div>
