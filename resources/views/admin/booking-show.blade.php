...
<div class="grid items-start gap-6 xl:grid-cols-[1fr_360px]">
    ...
    <aside class="card p-6">
        ...
        <div class="mt-5 space-y-3 text-sm"><div class="flex justify-between"><span class="text-slate-500">Status sewa</span><x-status-badge :status="$booking->status" /></div><div class="flex justify-between"><span class="text-slate-500">Status bayar</span><x-status-badge :status="$booking->payment_status" /></div><div class="flex justify-between border-t border-slate-100 pt-4"><span class="text-slate-500">Subtotal</span><span>Rp{{ number_format($booking->subtotal_amount, 0, ',', '.') }}</span></div><div class="flex justify-between"><span class="text-slate-500">Deposit</span><span>Rp{{ number_format($booking->deposit_amount, 0, ',', '.') }}</span></div><div class="flex justify-between"><span class="text-slate-500">Biaya layanan</span><span>Rp{{ number_format($booking->platform_fee, 0, ',', '.') }}</span></div>@if((float)$booking->late_fee > 0)<div class="flex justify-between text-rose-600"><span>Biaya terlambat</span><span>Rp{{ number_format($booking->late_fee,0,',','.') }}</span></div>@endif @if((float)$booking->damage_fee > 0)<div class="flex justify-between text-rose-600"><span>Biaya kerusakan</span><span>Rp{{ number_format($booking->damage_fee,0,',','.') }}</span></div>@endif<div class="flex justify-between border-t border-slate-100 pt-4 text-lg font-black text-ink"><span>Total</span><span>Rp{{ number_format($booking->total_amount, 0, ',', '.') }}</span></div></div>
