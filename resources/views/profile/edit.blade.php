...
@section('content')
@php
$profileFields = [$user->name, $user->email, $user->phone, $user->date_of_birth, $user->address, $user->city,
$user->province];
        ...
        <form action="{{ route('profile.update') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <section class="card p-6">
                <div class="flex flex-col gap-5 sm:flex-row sm:items-center">
                    @if($user->avatar_url)
                    <img src="{{ $user->avatar_url }}" class="h-24 w-24 rounded-3xl object-cover ring-4 ring-indigo-50"
                        alt="{{ $user->name }}">
                    @else
                    <span
                    ...
                    <div class="flex-1">
                        ...
                        <p class="mt-1 text-sm text-slate-500">{{ ucfirst($user->role) }} · Bergabung
                            ...
                            class="btn-secondary mt-4 inline-flex cursor-pointer py-2"><input type="file" name="avatar"
                                class="sr-only" accept=".jpg,.jpeg,.png">Ganti foto profil</label>
            ...
            <section class="card p-6">
                ...
                <div class="mt-6 grid gap-5 sm:grid-cols-2">
                    <div class="sm:col-span-2"><label class="label">Alamat lengkap</label><textarea name="address"
                            rows="3" class="input"
                            @required($user->role === 'customer')>{{ old('address', $user->address) }}</textarea></div>
                    ...
                    <div><label class="label">Provinsi</label><input name="province"
                            value="{{ old('province', $user->province) }}" class="input" @required($user->role ===
                        'customer')></div>
            ...
            <section class="card p-6">
                <div class="flex flex-wrap items-start justify-between gap-3">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-slate-500">Identitas tersimpan privat dan dapat digunakan saat
                            checkout.</p>
                    </div>@if($user->identity_file)<span
                ...
                <div class="mt-6 grid gap-5 sm:grid-cols-2">
                    <div><label class="label">Jenis identitas</label><select name="identity_type" class="input"
                            required>@foreach(['ktp' => 'KTP', 'sim' => 'SIM', 'kartu_pelajar' => 'Kartu
                            Pelajar/Mahasiswa', 'paspor' => 'Paspor'] as $value => $label)<option value="{{ $value }}"
                                @selected(old('identity_type', $user->identity_type) === $value)>{{ $label }}</option>
                            @endforeach</select></div>
                    <div><label class="label">Nomor identitas</label><input name="identity_number"
                            value="{{ old('identity_number', $user->identity_number) }}" class="input" required></div>
                    <div class="sm:col-span-2"><label
                            class="label">{{ $user->identity_file ? 'Ganti dokumen identitas' : 'Upload dokumen identitas' }}</label><input
                            type="file" name="identity_file" class="input" accept=".jpg,.jpeg,.png,.pdf" @required(!
                            $user->identity_file)><div
                            class="mt-2 flex flex-wrap items-center justify-between gap-2 text-xs"><span
                                class="text-slate-400">JPG, PNG, atau PDF maksimal 2
                                MB.</span>@if($user->identity_file)<a href="{{ route('profile.identity', $user) }}"
                                class="font-bold text-indigo-600">Buka dokumen saat ini →</a>@endif</div>
                    </div>
                </div>
            </section>
            ...
            <section class="card p-6">
                ...
                <p class="mt-1 text-sm text-slate-500">Dapat dihubungi jika terjadi kendala selama masa penyewaan.</p>
                <div class="mt-6 grid gap-5 sm:grid-cols-2">
                    <div><label class="label">Nama kontak</label><input name="emergency_contact_name"
                            value="{{ old('emergency_contact_name', $user->emergency_contact_name) }}" class="input">
                    </div>
                    <div><label class="label">Nomor HP kontak</label><input name="emergency_contact_phone"
                            value="{{ old('emergency_contact_phone', $user->emergency_contact_phone) }}" class="input">
                    </div>
                </div>
            </section>
            @endif
